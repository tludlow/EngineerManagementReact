...
// User actions: The below actions are asynchronous so they have a request and a
// success failure. We can't return the outcome in the result as the server wont
// reply Instantly. This is gotten around with by having mutliple outcomes a
// success and failure, which we fire in the thunk function as required.
...
// Function used to sign up the user. Dispatches action of request, sends the
// request and gets a response, then dispatches success or failure depending on
// needed reducer action.
...
export function loginUser(username, password) {
  return dispatch => {
    ...
    return axios
      .post('http://localhost:7770/user/login', {username, password})
      .then((response) => {
        if (response.data.error) {
